...
from weegit_add_ons.organoids.common import OrganoidsBaseAddOn, SpikesPayload


class SpikesViewerAddOn(OrganoidsBaseAddOn, BaseAddOn):
    ...
    def run(self, session_manager, add_on_data_dir):
        selected = self.choose_spikes_dir_dialog("Spike viewer", Path(add_on_data_dir))
        if selected is None:
            return
    ...
    def _load_payload(self, sweep_idx: int) -> Optional[SpikesPayload]:
        if not self.state.selected_spikes_dir:
            return None
        path = Path(self.state.selected_spikes_dir) / f"{int(sweep_idx)}.spikes.json"
        if not path.exists():
            return None
        ...
        for channel_idx, rect in channel_rects:
            ...
            for spike in spikes:
                if not (start_time_ms <= spike.time_ms <= end_time_ms):
                    continue
                x = ((float(spike.time_ms) - start_time_ms) / axis_duration_ms) * axis_width
